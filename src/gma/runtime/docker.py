...
def _docker(args: Sequence[str], timeout: float = 60.0) -> str:
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"docker command failed: {rendered}\n{result.stderr.strip()}")
...
def discover_containers(
    ...
    try:
        raw = _docker(["ps", "--format", fmt, "--filter", f"name={prefix}"])
    except RuntimeError:
        return []
...
def discover_backend_urls(
    ...
    containers = discover_containers(prefix)
...
def _verify_ready_webapps(name: str) -> None:
    # Check frontend web servers (must return 200)
    frontend_checks = (
        "http://localhost:8040",
        "http://localhost:8042",
        "http://localhost:8030",
    ...
    frontend_probe = " ".join(f'"{url}"' for url in frontend_checks)
    frontend_script = f"set -eu; for url in {frontend_probe}; do curl -fsS --max-time 8 \"$url\" >/dev/null; done"
    _docker(["exec", name, "sh", "-lc", frontend_script], timeout=90)

    # Check the same frontend-to-backend proxy route used by Android. Requiring
    # 200 catches an unhealthy backend and a broken nested-Docker network.
    xsl_script = "[ \"$(curl -s -o /dev/null -w '%{http_code}' --max-time 8 http://localhost:8030/api/health)\" = 200 ]"
    _docker(["exec", name, "sh", "-lc", xsl_script], timeout=30)
...
def _bootstrap_ready_container(name: str, backend_port: int, image: str) -> None:
    ...
    # Wait for the frontend API proxy before attempting browser login. This is
    # the complete route the WebView depends on, not only the backend listener.
    xsl_script = "[ \"$(curl -s -o /dev/null -w '%{http_code}' --max-time 8 http://localhost:8030/api/health)\" = 200 ]"
    ...
    while time.time() < deadline:
        try:
            ...
            continue
    if not xsl_ready:
        raise RuntimeError(f"XiaoShiLiu frontend API proxy did not become ready in {name}")
...
def launch_container(
    ...
    args.extend(
        [
            ...
            "-e",
            f"ENABLE_VNC={'true' if enable_vnc else 'false'}",
        ]
    )
    if privileged:
        args.append("--privileged")
    args.extend(["--entrypoint", "/usr/local/bin/entrypoint.sh"])
    args.extend(extra_args)
    args.extend([image, "tail", "-f", "/var/log/emulator.log", "/var/log/server.log"])
    cid = _docker(args, timeout=120)
    logger.info(
...
def exec_in_container(name: str, command: str, timeout: float = 60.0) -> str:
    ...
    return _docker(["exec", name, "bash", "-c", command], timeout=timeout)
