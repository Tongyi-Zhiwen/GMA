...
def _mastodon_web_exec(client: AndroidController, script: str, timeout: float = 180.0) -> str:
    ...
    return _run_bash(
        ...
        f"""
set -euo pipefail
cd {shlex.quote(MASTODON_PROJECT_DIR)}
cat <<'__GMA_MASTODON_WEB_B64__' | docker compose --project-name {shlex.quote(MASTODON_PROJECT_NAME)} -f docker-compose.yml exec -T web bash -lc 'base64 -d > /tmp/gma_web.sh && bash /tmp/gma_web.sh'
{payload}
__GMA_MASTODON_WEB_B64__
""",
        timeout=timeout,
    )
