...
from gma.runtime.client import DEFAULT_DEVICE, GMAClient
from gma.runtime.docker import DEFAULT_PREFIX, discover_backend_urls
    ...
    while step < max_steps:
        ...
        if action.action_type == ActionType.CALL_USER:
            ...
            traj.log_step(
                ...
                task_state_mode=agent.task_state_mode,
                state_update_response=agent.last_state_response,
                state_update_error=agent.last_state_error,
                image_caption=agent.last_image_caption,
                image_caption_response=agent.last_image_caption_response,
                image_caption_error=agent.last_image_caption_error,
                image_caption_mode=agent.image_caption_mode,
...
def _default_container_name_for_url(
    ...
    try:
        ...
        return f"{DEFAULT_PREFIX}_{index}"
    env_id = port - 8100
    if env_id >= 0:
        return f"{DEFAULT_PREFIX}_{env_id}"
    return f"{DEFAULT_PREFIX}_{index}"
...
def run_evaluation(
    ...
    # Create clients
    clients = [
        GMAClient(url, device=DEFAULT_DEVICE, step_wait_time=step_wait_time)
