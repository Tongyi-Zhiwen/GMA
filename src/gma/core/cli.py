...
from gma.runtime.client import DEFAULT_DEVICE
from gma.runtime.docker import DEFAULT_IMAGE
...
def create_parser() -> argparse.ArgumentParser:
    ...
    up.add_argument("--image", default=DEFAULT_IMAGE, help="Docker image")
    ...
    reset_p.add_argument("--device", default=DEFAULT_DEVICE)
    ...
    clear_p.add_argument("--device", default=DEFAULT_DEVICE)
    ...
    baseline_p.add_argument("--device", default=DEFAULT_DEVICE)
    ...
    ev.add_argument(
        "--config",
        default=None,
        help="Config TOML path (default: configs/config.toml)",
    )
    ...
    ev.add_argument(
        "--robust",
        action="store_true",
        help="Enable robust environment recovery with retries",
    )
    ev.add_argument(
        "--max-task-retries",
        type=int,
        default=3,
        help="Max retries per task when robust recovery is enabled (default: 3)",
    )
    ev.add_argument(
        "--recovery-timeout",
        type=float,
        default=420.0,
        help="Timeout in seconds for environment recovery (default: 420)",
    )
    ...
    ts.add_argument("--robust", action="store_true", help="Enable robust environment recovery")
    ts.add_argument("--max-task-retries", type=int, default=3)
    ts.add_argument("--recovery-timeout", type=float, default=420.0)
    ...
    manual.add_argument("--device", default=DEFAULT_DEVICE)
...
def _add_agent_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--agent-profile", default=None, help="Named profile in configs/config.toml")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of recent screenshots retained in agent context (default: 2)",
    )
    parser.add_argument(
        "--action-only-history",
        action="store_true",
        help="Keep only normalized actions, without reasoning, in assistant history",
    )
    parser.add_argument(
        "--caption-old-images",
        action="store_true",
        help=(
            "Replace screenshots older than --last-images with model-generated "
            "historical captions (qwen_agent and claude_agent)"
        ),
    )
    parser.add_argument(
        "--freeform-state",
        action="store_true",
        help="Maintain a latest-only free-form task-state record",
    )
    parser.add_argument(
        "--structured-state",
        action="store_true",
        help="Maintain structured task state using explicit before/after screenshots",
    )
    parser.add_argument(
        "--claude",
        choices=("claude",),
        default=None,
        metavar="claude",
        help="Enable claude_agent's Claude coordinate scaling: --claude claude",
    )
    parser.add_argument(
        "--doubao",
        choices=("doubao",),
        default=None,
        metavar="doubao",
        help="Enable claude_agent's normalized 0-1000 coordinates: --doubao doubao",
    )
    ...
    if args.env_action in {"reset", "clear", "save-backend-baseline"}:
        ...
        if args.env_action == "save-backend-baseline":
            ...
            specs = [
                ...
                (offline_backend_baseline(TRAVEL), lambda c: ensure_offline_webapp_backend(c, TRAVEL)),
            ]
...
def cmd_eval(args):
    ...
    doubao = getattr(args, "doubao", None)
    last_images = getattr(args, "last_images", None)
    action_only_history = getattr(args, "action_only_history", False)
    caption_old_images = getattr(args, "caption_old_images", False)
    freeform_state = getattr(args, "freeform_state", False)
    structured_state = getattr(args, "structured_state", False)
    if freeform_state and structured_state:
        raise ValueError(
            "--freeform-state and --structured-state are mutually exclusive"
        )
    if claude and agent_type != "claude_agent":
        raise ValueError("--claude claude is only supported by --agent-type claude_agent")
    if doubao and agent_type != "claude_agent":
        raise ValueError("--doubao doubao is only supported by --agent-type claude_agent")
    if caption_old_images and agent_type not in {"claude_agent", "qwen_agent"}:
        raise ValueError(
            "--caption-old-images is only supported by qwen_agent and claude_agent"
        )
    if claude and doubao:
        raise ValueError("--claude and --doubao are mutually exclusive")
    if last_images is not None and last_images < 1:
        raise ValueError("--last-images must be at least 1")
    ...
    agent_kwargs = {
        ...
        "claude": claude,
        "doubao": doubao,
        "action_only_history": action_only_history,
        "caption_old_images": caption_old_images,
        "freeform_state": freeform_state,
        "structured_state": structured_state,
    }
    if last_images is not None:
        agent_kwargs["last_images"] = last_images
    ...
    results = run_evaluation(
        ...
        robust_env_recovery=args.robust,
        robust_env_max_task_retries=args.max_task_retries,
        robust_env_recovery_timeout=args.recovery_timeout,
    )
