...
class ElementXRecommendedProductXslMallBuyTask(BaseTask):
    ...
    difficulty = "cross-application"
    snapshot = "gma_ready_state"
    assets = (
        ElementXUserAsset(username="w4-row227-friend", password="password", display_name="Product Friend"),
        ElementXRoomAsset(name="Product Friend", room_type="dm", creator_username="w4-row227-friend", creator_password="password", members=["testuser"]),
        ElementXMessageAsset(room=elementx_user_id("w4-row227-friend"), sender_username="w4-row227-friend", sender_password="password", text=RECOMMENDATION),
