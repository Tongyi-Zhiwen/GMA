...
def clear_environment(client, apps: set[str] | None = None) -> bool:
    ...
    try:
        ...
        if requested("Contacts"):
            try:
                client.shell(
                    "pm grant com.google.android.contacts "
                    "android.permission.POST_NOTIFICATIONS >/dev/null 2>&1 || true"
                )
            except Exception:
                logger.warning("Contacts notification permission grant failed during reset")
...
adb -s "$DEVICE" root >/dev/null 2>&1 || true
adb -s "$DEVICE" shell "printf '%s\n' 'chrome --disable-vulkan --disable-features=Vulkan --unsafely-treat-insecure-origin-as-secure=http://10.0.2.2:8050,http://10.0.2.2:8040,http://10.0.2.2:8030,http://10.0.2.2:8070,http://10.0.2.2:8060' > /data/local/tmp/chrome-command-line" >/dev/null 2>&1 || true
...
def ensure_ready_state_apps(
    ...
    def reset_and_ensure_xiaoshiliu(client):
        ...
        # Require the same frontend API proxy path used by the Android WebView.
        login_ready_script = """
set -euo pipefail
for i in $(seq 1 30); do
  code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 5 http://localhost:8030/api/health 2>/dev/null || echo "000")
  if [ "$code" = "200" ]; then
    echo "Frontend API proxy ready (HTTP $code)"
    exit 0
  fi
  sleep 2
done
echo "Frontend API proxy not ready after 60s"
exit 1
"""
        run_bash(client, login_ready_script, timeout=90)
