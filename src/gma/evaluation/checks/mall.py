...
class MallCheckoutOrderCreated(Criterion):
    def __init__(
        ...
        *,
        ...
        receiver_detail_address: str | None = None,
        weight: float = 1.0,
    ):
        super().__init__(weight=weight)
        self.member_username = member_username
        ...
        self.receiver_detail_address = receiver_detail_address
    ...
    def evaluate(self, client) -> Any:
        ...
        expected_receiver_fields = {
            ...
            "receiver_detail_address": self.receiver_detail_address,
        }
        for field, expected in expected_receiver_fields.items():
            if expected is not None and order.get(field) != expected:
                return self._fail(f"{field} mismatch. Expected {expected}, current: {order}")
