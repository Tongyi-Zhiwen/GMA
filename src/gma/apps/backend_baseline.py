...
# Retry docker-compose up to handle transient MySQL healthcheck failures under load.
compose_up_with_retry() (
  local retries=3
  local delay=15
  local attempt=1
  while [ $attempt -le $retries ]; do
    if {spec.compose_up} >/dev/null 2>&1; then
      return 0
    fi
    if [ $attempt -lt $retries ]; then
      echo "Compose up attempt $attempt failed, retrying in $delay seconds..." >&2
      sleep $delay
    fi
    attempt=$((attempt + 1))
  done
...
if [ -f "$PROJECT_DIR/docker-compose.yml" ]; then
  (
    ...
    # shellcheck disable=SC2086
    docker compose $compose_files config --images | sort -u
  ) > "$TMP/images.txt" || : > "$TMP/images.txt"
fi
