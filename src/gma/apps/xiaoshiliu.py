...
# Probe the same frontend proxy path that Android uses. A direct backend login
# creates a real session and can race with the browser login below.
with urllib.request.urlopen("http://localhost:8030/api/health", timeout=20) as response:
    if response.status != 200:
        raise RuntimeError(f"XiaoShiLiu frontend API proxy is not ready: HTTP {response.status}")
...
if not pid:
    ...
    while time.time() < deadline:
        ...
        time.sleep(1)
if not pid:
    raise RuntimeError("XiaoShiLiu WebView package did not start after explicit relaunch")
    ...
    while time.time() < deadline:
        try:
            ...
            if not page and pages:
                # It is safe to attach to the wrapper's current page only to
                # navigate it. The target origin is verified before login.
                page = pages[0]
            if page:
                return page
...
async def wait_until_target_page(websocket, start_message_id, *, timeout=30):
    ...
    while time.time() < deadline:
        try:
            ...
            if (
                str(last.get("href") or "").startswith("http://10.0.2.2:8030")
                and last.get("readyState") in ("interactive", "complete")
            ):
                return message_id + 1
        except Exception as exc:
            last = {"error": repr(exc)}
    ...
    raise RuntimeError(
        "XiaoShiLiu WebView could not reach http://10.0.2.2:8030; "
        + "check Docker-in-Docker NAT/iptables support: "
...
(async function () {
  ...
  try {
    ...
    throw new Error('XiaoShiLiu browser login request failed at ' + location.href + ': ' + String(error));
  }
...
async def sync_frontend_session():
    ...
    async with websockets.connect(websocket_url, max_size=None) as websocket:
        ...
        message_id += 1
        message_id = await wait_until_target_page(websocket, message_id, timeout=30)
        login_result = await evaluate(websocket, message_id, login_script, timeout=30)
        message_id += 1
        # Reload the authenticated route once so the SPA reads the newly
        # written localStorage values. Do not inject a persistent login script:
        # retries and navigations must never create concurrent sessions.
        await navigate(websocket, message_id, "http://10.0.2.2:8030/user", timeout=20)
        message_id += 1
        message_id = await wait_until_target_page(websocket, message_id, timeout=30)
