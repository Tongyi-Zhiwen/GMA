...
class OfflineWebApp:
    ...
    archive_prefixes: tuple[str, ...] = ("/app/dev",)
...
travel_dump=/tmp/gma_travel_export/travel-app-migration-pack/db-backup/travel-db-dump.tar.gz
needs_flight_catalog_restore=$(docker exec travel-mongo mongosh --quiet -u travel -p travel_root_2025 --authenticationDatabase admin travel --eval '
const scheduled = db.flightitineraries.countDocuments({status: "scheduled"});
const segments = db.flightsegments.countDocuments({});
const seats = db.flightseats.countDocuments({});
print((scheduled < 1000 || segments < 1000 || seats < 10000 || segments < scheduled || seats < segments) ? "yes" : "no");
')
if [ "$needs_flight_catalog_restore" = "yes" ] && [ -f "$travel_dump" ]; then
  rm -rf /tmp/gma_travel_reference_restore /tmp/gma_travel_flight_refs
  mkdir -p /tmp/gma_travel_reference_restore /tmp/gma_travel_flight_refs
  tar -xzf "$travel_dump" -C /tmp/gma_travel_reference_restore --strip-components=1
  for collection in airlines airports airplanes airlineflightprices seats flightitineraries flightsegments flightseats flightreviews; do
    if [ -f "/tmp/gma_travel_reference_restore/travel/${collection}.bson" ]; then
      cp "/tmp/gma_travel_reference_restore/travel/${collection}.bson" /tmp/gma_travel_flight_refs/
    fi
    if [ -f "/tmp/gma_travel_reference_restore/travel/${collection}.metadata.json" ]; then
      cp "/tmp/gma_travel_reference_restore/travel/${collection}.metadata.json" /tmp/gma_travel_flight_refs/
    fi
  done
  docker exec travel-mongo rm -rf /tmp/gma_travel_flight_refs
  docker cp /tmp/gma_travel_flight_refs travel-mongo:/tmp/gma_travel_flight_refs >/dev/null
  docker exec travel-mongo mongorestore -u travel -p travel_root_2025 --authenticationDatabase admin --db travel --drop --noIndexRestore /tmp/gma_travel_flight_refs --quiet
fi
...
const futureTripDate = new Date('2026-10-05T00:00:00.000Z');
const futureTripExpireAt = new Date(futureTripDate.getTime() - 30 * 60 * 1000);
...
if (!ekAirplane || !ekAirplane._id) {
  throw new Error('Travel reference repair requires at least one airplane');
}
const ekAirplaneId = ekAirplane._id;
let ekAirplaneSeats = Array.isArray(ekAirplane.seats) ? ekAirplane.seats : [];
if (!ekAirplaneSeats.length) {
  ekAirplaneSeats = [
    {seatNumber: '52H', class: 'economy'},
    {seatNumber: '52J', class: 'economy'},
    {seatNumber: '52K', class: 'economy'},
    {seatNumber: '6D', class: 'business'},
  ];
}
const baggageAllowance = {
  ...
  carryOn: {
    ...
    maxDimensions: {length: 55, width: 38, height: 20, measurementUnit: 'cm'},
  },
  checked: {
    maxPieces: 1,
    maxWeight: {measurementUnit: 'kg', value: 23},
    maxDimensions: {length: 158, width: 0, height: 0, measurementUnit: 'cm'},
  },
};
function passengerPrices(adult, child, infant) {
  return {adult, child, infant};
}
function passengerPercent(amount) {
  return {
    adult: {type: 'percentage', amount},
    child: {type: 'percentage', amount},
    infant: {type: 'percentage', amount},
  };
}
function fareDetails(baseEconomy) {
  return {
    airlineCode: 'EK',
    departureAirportCode: 'DXB',
    arrivalAirportCode: 'LHR',
    distance: {lengthIn: 'mi', value: 3420},
    basePrice: {
      economy: passengerPrices(baseEconomy, Math.round(baseEconomy * 0.75), 80),
      premium_economy: passengerPrices(Math.round(baseEconomy * 1.6), Math.round(baseEconomy * 1.2), 120),
      business: passengerPrices(Math.round(baseEconomy * 3.2), Math.round(baseEconomy * 2.4), 180),
      first: passengerPrices(Math.round(baseEconomy * 5.2), Math.round(baseEconomy * 3.9), 250),
    },
    discount: passengerPercent(0),
    serviceFee: passengerPercent(0),
    taxes: passengerPercent(0),
  };
}
function objectIdFromPrefix(prefix, index) {
  const value = prefix + index.toString(16).padStart(2, '0');
  if (value.length !== 24) throw new Error('Invalid generated ObjectId: ' + value);
  return ObjectId(value);
}
function ensureFlightSeats(segmentId, expireAt, seatPrefix) {
  const seatDocs = ekAirplaneSeats.map((seat, index) => ({
    _id: objectIdFromPrefix(seatPrefix, index),
    seatNumber: String(seat.seatNumber || `${index + 1}A`),
    airplaneId: ekAirplaneId,
    class: seat.class || 'economy',
    segmentId,
    reservation: {pnrCode: null, for: null, type: null, expiresAt: null},
    expireAt,
    updatedAt: new Date(),
  }));
  if (seatDocs.length) {
    db.flightseats.bulkWrite(seatDocs.map(seat => ({
      updateOne: {
        filter: {_id: seat._id},
        update: {$set: seat, $setOnInsert: {createdAt: new Date()}},
        upsert: true,
      },
    })), {ordered: false});
    db.flightseats.deleteMany({segmentId, _id: {$nin: seatDocs.map(seat => seat._id)}});
  }
  return seatDocs.map(seat => seat._id);
}
function ensureReferenceFlight({itineraryId, segmentId, seatPrefix, flightCode, flightNumber, date, expireAt, status, baseEconomy, departureOffsetMinutes = 0}) {
  const departure = new Date(date.getTime() + departureOffsetMinutes * 60 * 1000);
  const arrival = new Date(departure.getTime() + (9 * 60 * 60 * 1000 + 56 * 60 * 1000));
  const seatIds = ensureFlightSeats(segmentId, expireAt, seatPrefix);
  db.flightsegments.updateOne({_id: segmentId}, {$set: {
    flightNumber,
    airlineId: 'EK',
    date: departure,
    from: {airport: 'DXB', scheduledDeparture: departure, terminal: '3', gate: 'B22'},
    to: {airport: 'LHR', scheduledArrival: arrival, terminal: '3'},
    durationMinutes: 596,
    airplaneId: ekAirplaneId,
    seats: seatIds,
    fareDetails: fareDetails(baseEconomy),
    baggageAllowance,
    status,
    expireAt,
    updatedAt: new Date(),
...
ensureReferenceFlight({
  itineraryId: ObjectId('6a04821d3603c9908e3d1ff8'),
  segmentId: ObjectId('6a04821d3603c9908e3708b0'),
  seatPrefix: '6a04821d3603c9908e3710',
  flightCode: 'EK2106',
...
ensureReferenceFlight({
  itineraryId: ObjectId('6a04821d3603c9908e3d1ff9'),
  segmentId: ObjectId('6a04821d3603c9908e3708b1'),
  seatPrefix: '6a04821d3603c9908e3720',
  flightCode: 'EK2106',
  flightNumber: 'EK2106-2',
  date: futureTripDate,
  expireAt: futureTripExpireAt,
  status: 'scheduled',
  baseEconomy: 650,
});
ensureReferenceFlight({
  itineraryId: ObjectId('6a04821d3603c9908e3d1ffa'),
  segmentId: ObjectId('6a04821d3603c9908e3708b2'),
  seatPrefix: '6a04821d3603c9908e3730',
  flightCode: 'EK2156',
  flightNumber: 'EK2156-1',
  date: futureTripDate,
  expireAt: futureTripExpireAt,
  status: 'cancelled',
  baseEconomy: 610,
...
ensureReferenceFlight({
  itineraryId: ObjectId('6a04821d3603c9908e3d1ffb'),
  segmentId: ObjectId('6a04821d3603c9908e3708b3'),
  seatPrefix: '6a04821d3603c9908e3740',
  flightCode: 'EK2136',
  flightNumber: 'EK2136-1',
  date: futureTripDate,
  expireAt: futureTripExpireAt,
  status: 'scheduled',
  baseEconomy: 540,
...
const fallbackReviewer = db.users.findOne({})?._id || ObjectId('6a014f091a3d4a9f4844ba89');
db.flightreviews.updateOne(
  {_id: ObjectId('6a04821d3603c9908e3d1ffc')},
  {$set: {
    airlineId: 'EK',
    departureAirportId: 'DXB',
    arrivalAirportId: 'LHR',
    airplaneModelName: ekAirplane.model || 'Airbus A380',
    reviewer: fallbackReviewer,
    rating: 4.6,
    title: 'Reliable Dubai to London service',
    review: 'Smooth boarding and clear timing on the Dubai to London route.',
    updatedAt: new Date(),
  }, $setOnInsert: {_id: ObjectId('6a04821d3603c9908e3d1ffc'), createdAt: new Date()}},
  {upsert: true},
);
...
for (const collectionName of ['flightitineraries', 'flightsegments', 'flightseats']) {
  dropIndexIfExists(collectionName, 'expireAt_1');
}
db.flightitineraries.createIndex(
  {departureAirportId: 1, arrivalAirportId: 1, date: 1, status: 1},
  {name: 'gma_route_date_status_lookup'},
);
db.flightitineraries.createIndex(
  {expireAt: 1, date: 1},
  {name: 'gma_expireAt_date_lookup'},
);
db.flightsegments.createIndex(
  {date: 1, status: 1},
  {name: 'gma_segment_date_status_lookup'},
);
db.flightsegments.createIndex(
  {expireAt: 1, date: 1},
  {name: 'gma_segment_expireAt_date_lookup'},
);
db.flightseats.createIndex(
  {segmentId: 1},
  {name: 'gma_seat_segment_lookup'},
);
db.flightseats.createIndex(
  {expireAt: 1},
  {name: 'gma_seat_expireAt_lookup'},
);
JS
docker cp /tmp/gma_travel_reference.js travel-mongo:/tmp/gma_travel_reference.js >/dev/null
docker exec travel-mongo mongosh --quiet -u travel -p travel_root_2025 --authenticationDatabase admin travel /tmp/gma_travel_reference.js >/dev/null
""",
        timeout=900,
    )
...
if [ "{app.label}" = "Travel" ]; then
  ...
  if [ -f db-backup/travel-db-dump.tar.gz ]; then
    ...
    if [ -d /tmp/travel-dump/travel ]; then
      ...
      docker exec travel-mongo mongorestore -u travel -p travel_root_2025 --authenticationDatabase admin --db travel --drop --noIndexRestore /tmp/travel-dump --quiet
    fi
  fi
fi
...
# Older Travel images may contain a legacy IPv4 origin in compiled Next.js
# files. Discover that host and rewrite it to the emulator's local gateway.
docker exec -u root travel-app sh -lc '
  set -eu
  legacy_host=$(
    find /app/.next -type f \( -name "*.js" -o -name "*.html" -o -name "*.json" \) -print0 \
      | xargs -0 grep -ahoE "http://([0-9]{1,3}\\.){3}[0-9]{1,3}(:8060)?/trip" \
      | sed -E "s#http://([^/:]+).*#\\1#" \
      | grep -v "^10\\.0\\.2\\.2$" \
      | head -n 1 || true
  )
  if [ -n "$legacy_host" ]; then
    legacy_pattern=$(printf "%s" "$legacy_host" | sed "s/\\./\\\\./g")
    find /app/.next -type f \( -name "*.js" -o -name "*.html" -o -name "*.json" \) -print0 \
      | xargs -0 sed -E -i "s#http://$legacy_pattern(:8060)?/trip#http://10.0.2.2:8060/trip#g; s#$legacy_pattern:8060#10.0.2.2:8060#g; s#$legacy_pattern#10.0.2.2#g"
    for file in /app/server.js /app/.next/required-server-files.json; do
      [ -f "$file" ] || continue
      sed -E -i "s#$legacy_pattern:8060#10.0.2.2:8060#g; s#$legacy_pattern#10.0.2.2#g" "$file"
    done
  fi
'
...
NODE
# The flight booking passenger form used the logged-in account email as a
# disabled controlled value for the primary traveler. Tasks may require a
# passenger contact email that differs from the login email. Make the field an
# editable uncontrolled input like the other passenger text fields, while still
# seeding it from the login email and persisting changes through onChange.
docker exec -i -u root travel-app node - <<'NODE'
const fs = require("fs");
const path = require("path");
function walk(root, out = []) {
...
const oldPattern = /value:([A-Za-z_$][\w$]*)\|\|([A-Za-z_$][\w$]*)\?\.email,type:"text",name:([A-Za-z_$][\w$]*)\+"-email",label:"Email",placeholder:"Email",onChange:([A-Za-z_$][\w$]*),required:!0,error:([A-Za-z_$][\w$]*)\?\.email,disabled:!!\1}/g;
const editableControlledPattern = /value:null==([A-Za-z_$][\w$]*)\?\.email\?([A-Za-z_$][\w$]*)\|\|"":\1\.email,type:"text",name:([A-Za-z_$][\w$]*)\+"-email",label:"Email",placeholder:"Email",onChange:([A-Za-z_$][\w$]*),required:!0,error:([A-Za-z_$][\w$]*)\?\.email,disabled:!1}/g;
const fixedPattern = /defaultValue:([A-Za-z_$][\w$]*)\?\.email\|\|([A-Za-z_$][\w$]*)\|\|"",type:"text",name:([A-Za-z_$][\w$]*)\+"-email",label:"Email",placeholder:"Email",onChange:([A-Za-z_$][\w$]*),required:!0,error:([A-Za-z_$][\w$]*)\?\.email,disabled:!1}/;
let patched = 0;
let alreadyPatched = 0;
function replacement(primaryEmailVar, passengerStateVar, keyVar, onChangeVar, errorVar) {
  return `defaultValue:${passengerStateVar}?.email||${primaryEmailVar}||"",type:"text",name:${keyVar}+"-email",label:"Email",placeholder:"Email",onChange:${onChangeVar},required:!0,error:${errorVar}?.email,disabled:!1}`;
}
for (const file of [...walk("/app/.next/static"), ...walk("/app/.next/server")]) {
  let text = fs.readFileSync(file, "utf8");
  if (!text.includes('primaryPassengerEmail') && !text.includes('passengersDetails')) continue;
  if (fixedPattern.test(text)) alreadyPatched += 1;
  const before = text;
  text = text.replace(oldPattern, (_match, primaryEmailVar, passengerStateVar, keyVar, onChangeVar, errorVar) => (
    replacement(primaryEmailVar, passengerStateVar, keyVar, onChangeVar, errorVar)
  ));
  text = text.replace(editableControlledPattern, (_match, passengerStateVar, primaryEmailVar, keyVar, onChangeVar, errorVar) => (
    replacement(primaryEmailVar, passengerStateVar, keyVar, onChangeVar, errorVar)
  ));
  if (text !== before) {
    fs.writeFileSync(file, text);
...
if (patched === 0 && alreadyPatched === 0) {
  throw new Error("Travel editable passenger email patch did not match any bundle");
}
NODE
...
NODE
# Flight/hotel review rendering assumes all historical reviews have reviewer
# ids. Guard missing reviewers so a single legacy review cannot trip the page.
docker exec -i -u root travel-app node - <<'NODE'
const fs = require("fs");
const path = require("path");
function walk(root, out = []) {
...
for (const file of walk("/app/.next/server")) {
  let text = fs.readFileSync(file, "utf8");
  if (!text.includes("FlightOrHotelReviewList") && !text.includes("flightReviews")) continue;
  const before = text;
  text = text.split('/*__gmaReviewerGuard*/)}').join('/*__gmaReviewerGuard*/)');
  if (text.includes("__gmaReviewerGuard")) {
    alreadyPatched += 1;
  } else {
    text = text.replace(/([A-Za-z_$][\w$]*)\.reviewer\.toString\(\)===([A-Za-z_$][\w$]*)\?\.user\.id/g, (_match, reviewVar, sessionVar) => {
      patched += 1;
      return `${reviewVar}.reviewer&&${reviewVar}.reviewer.toString&&${reviewVar}.reviewer.toString()===${sessionVar}?.user?.id/*__gmaReviewerGuard*/`;
    });
  }
  if (text !== before) {
    fs.writeFileSync(file, text);
...
if (patched === 0 && alreadyPatched === 0) {
  throw new Error("Travel review reviewer guard patch did not match any server bundle");
}
NODE
# The flight search result page computes filter ranges in a separate server
# bundle before the client results render. Guard that fare calculation too,
# because that bundle can receive unpopulated segmentIds from Mongoose.
docker exec -i -u root travel-app node - <<'NODE'
const fs = require("fs");
const path = require("path");
function walk(root, out = []) {
...
const resultFilterPattern = /([A-Za-z_$][\w$]*)\.forEach\(([A-Za-z_$][\w$]*)=>\{let\{total:([A-Za-z_$][\w$]*)\}=\(0,([A-Za-z_$][\w$]*)\.F_\)\(\2\.segmentIds,\{adult:([A-Za-z_$][\w$]*)\.adults,child:\5\.children,infant:\5\.infants\},([A-Za-z_$][\w$]*)\);([A-Za-z_$][\w$]*)\.push\(\3\),([A-Za-z_$][\w$]*)\.add\(\2\.carrierInCharge\._id\)\}\)/g;
let patched = 0;
let alreadyPatched = 0;
for (const file of walk("/app/.next/server")) {
  let text = fs.readFileSync(file, "utf8");
  if (!text.includes("FlightItinerary") || !text.includes("FlightFilter")) continue;
  const before = text;
  if (text.includes("__gmaValidResultFlightSegments")) {
    alreadyPatched += 1;
    continue;
  }
  text = text.replace(
    resultFilterPattern,
    (_match, listVar, flightVar, totalVar, fareModuleVar, passengerVar, classVar, totalsVar, airlinesVar) => {
      patched += 1;
      return `${listVar}.forEach(${flightVar}=>{let __gmaValidResultFlightSegments=Array.isArray(${flightVar}.segmentIds)&&${flightVar}.segmentIds.length>0&&${flightVar}.segmentIds.every(__gmaSegment=>__gmaSegment&&__gmaSegment._id&&__gmaSegment.fareDetails)&&${flightVar}.carrierInCharge&&${flightVar}.carrierInCharge._id;if(!__gmaValidResultFlightSegments)return;let{total:${totalVar}}=(0,${fareModuleVar}.F_)(${flightVar}.segmentIds,{adult:${passengerVar}.adults,child:${passengerVar}.children,infant:${passengerVar}.infants},${classVar});${totalsVar}.push(${totalVar}),${airlinesVar}.add(${flightVar}.carrierInCharge._id)})`;
    }
  );
  if (text !== before) {
    fs.writeFileSync(file, text);
...
if (patched === 0 && alreadyPatched === 0) {
  throw new Error("Travel flight result filter patch did not match any server bundle");
}
NODE
...
if [ "${travel_ready:-0}" != "1" ]; then
  docker logs --tail=80 travel-app >&2 || true
  exit 1
fi
# Stop the Android Travel wrapper and clear WebView caches. Runtime patches keep
# the original Next chunk filenames, so an already-open WebView can otherwise
# keep a stale booking page with disabled passenger contact fields alive.
adb shell am force-stop gma.webapp.travel >/dev/null 2>&1 || true
adb root >/dev/null 2>&1 || true
adb wait-for-device >/dev/null 2>&1 || true
adb shell su 0 'rm -rf \
  /data/user/0/gma.webapp.travel/cache/* \
  /data/user/0/gma.webapp.travel/code_cache/* \
  /data/user/0/gma.webapp.travel/app_webview/Default/Cache \
  /data/user/0/gma.webapp.travel/app_webview/Default/Code\ Cache \
  /data/user/0/gma.webapp.travel/app_webview/Default/Service\ Worker \
  /data/user/0/gma.webapp.travel/app_webview/Default/GPUCache \
  /data/user/0/gma.webapp.travel/app_webview/Default/Session\ Storage \
  /data/user/0/gma.webapp.travel/app_webview/Default/Local\ Storage/leveldb/*' >/dev/null 2>&1 || true
exit 0
""",
        timeout=180,
    )
...
if [ -n "$seed_root" ] && docker ps --format '{{.Names}}' | grep -qx mall-mysql; then
  if [ -d "$seed_root/images/mall" ]; then
    docker exec mall-app-web mkdir -p /usr/share/nginx/html/gma-cached-assets/mall
    docker cp "$seed_root/images/mall/." mall-app-web:/usr/share/nginx/html/gma-cached-assets/mall/ >/dev/null
  fi
...
def patch_mall_address_form(container: str) -> None:
    ...
    covert_v9 = (
        'covertAdderss:function(e){console.log("covertAdderss",e),e=e.trim();var t=e.indexOf("Sheng");'
        'if(-1!=t){this.addressData.province=e.substring(0,t+5).trim();var r=e.substring(t+5).trim(),n=r.indexOf("Shi");'
        '-1!=n?(this.addressData.city=r.substring(0,n+3).trim(),this.addressData.region=r.substring(n+3).trim()):'
        '(this.addressData.city="",this.addressData.region=r)}else{var a=e.indexOf("Shi");'
        '-1!=a?(this.addressData.province=e.substring(0,a+3).trim(),this.addressData.city=this.addressData.province,this.addressData.region=e.substring(a+3).trim()):'
        '(this.addressData.province=e,this.addressData.city="",this.addressData.region="")}},confirm:function(){'
    )
    covert_new = (
        'covertAdderss:function(e){var t=(e||"").trim(),r;console.log("covertAdderss",t),'
        '-1!=t.indexOf("省")?'
...
if [ -n "$seed_root" ] && docker ps --format '{{.Names}}' | grep -qx meituan-mongo; then
  if [ -d "$seed_root/images/meituan" ]; then
    docker exec meituan-frontend mkdir -p /usr/share/nginx/html/gma-cached-assets/meituan
    docker cp "$seed_root/images/meituan/." meituan-frontend:/usr/share/nginx/html/gma-cached-assets/meituan/ >/dev/null
  fi
...
# Login is performed exactly once by login_xiaoshiliu_app after the WebView
# reaches the frontend. Remove bootstraps left by older ready images so they do
# not race with, or switch users underneath, that explicit login.
...
def reset_xiaoshiliu_backend(client: AndroidController) -> None:
    ...
    # Clear stale user sessions to avoid duplicate token errors on login
    run_bash(
        client,
        """
set -euo pipefail
docker exec xiaoshiliu-mysql mysql -uxiaoshiliu_user -p123456 xiaoshiliu \
  -e "DELETE FROM user_sessions;" >/dev/null 2>&1 || true
""",
        timeout=30,
    )
